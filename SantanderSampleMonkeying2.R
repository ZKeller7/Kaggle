...
boost.mod <- xgboost(params              = ParamSet, 
                     ...
                     nrounds             = 150, #from cross validation
                     verbose             = F,
                     maximize            = TRUE,
                     stratified          = T)
...
ConfusionMat <- confusionMatrix(BinaryPreds, ValLabs)
ConfusionMat
...
write.csv(submission, file = "E:/Downloads/Santander/37.csv", row.names = FALSE)
save(boost.mod, file = "E:/Downloads/Santander/mod37.RData")
