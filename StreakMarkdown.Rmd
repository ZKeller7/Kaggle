...
---
**[Link to GitHub Code](https://github.com/ZKeller7/ML/blob/master/StreakForTheCashModel.R)**
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Detroit Pistons or Washington Wizards?

Another example from March 12:

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;What will be the GAME RESULT?:  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Georgia: Wins or Single Digit Loss or Kentucky: Wins By Double Digits?
...
First we need to collect data. I will use R for this project; using the XML package we can scrape data tables from ESPN's website. Each table contains the entire list of matchups from any particular day, including the teams playing, the sport, the vote total for each side, and a few other features. We can use the following code to scrape:
...
Here we can answer the first part of our question. We can see that ESPN users clearly should not attempt to take their talents to Las Vegas. The likelihood that the majority vote on any particular matchup is the correct choice is only 51.67%, barely better than flipping a coin! Still, there may yet be fruit in the search for the answer to our second question - can we build a model that will tell us, based on features that we have before the matchup is locked, if it is likely to be predicted correctly?
